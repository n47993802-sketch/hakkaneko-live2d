function updateAnimFields() {
  const isSticker =
    document.querySelector('input[name="animBaseType"]:checked').value ===
    "sticker";
...
function calculateAnim() {
  ...
  if (isSticker) {
    let qty = Math.min(
      99,
      Math.max(1, parseInt(document.getElementById("stickerQty").value) || 1),
    );
    let physics =
      parseInt(document.getElementById("stickerPhysics").value) || 0;
    let time = Math.min(
      60,
      Math.max(1, parseInt(document.getElementById("stickerTime").value) || 3),
    );
    let baseSticker = 400;
    details.push({
      name: `${_d2.anim_type_sticker || "動態貼圖"} x${qty}`,
      price: baseSticker * qty,
    });
    if (physics > 0)
      details.push({
        name: _d2.physics_high_200 || "高精細度",
        price: physics * qty,
      });
    if (time > 3)
      details.push({
        name: `${_d2.field_duration || "動態時間"} +${time - 3}s x${qty}`,
        price: (time - 3) * 50 * qty,
      });
    extra += (baseSticker + physics + (time > 3 ? (time - 3) * 50 : 0)) * qty;
  } else {
    let physics =
      parseInt(document.getElementById("processPhysics").value) || 0;
    let time = Math.min(
      600,
      Math.max(1, parseInt(document.getElementById("processTime").value) || 5),
    );
    let chars = Math.min(
      20,
      Math.max(1, parseInt(document.getElementById("processChars").value) || 1),
    );
    let baseCost = 2500;
    details.push({
      name: `${_d2.anim_type_process || "動畫處理"}`,
      price: baseCost,
    });
    if (physics > 0) {
      const physicsLabel =
        physics === 1000
          ? _d2.physics_high_1000 || "高精細度"
          : physics === 2000
            ? _d2.physics_ultra || "極致精細"
            : `+${physics}`;
      details.push({ name: physicsLabel, price: physics });
    }
    if (time > 5)
      details.push({
        name: `${_d2.field_duration || "動畫時長"} +${time - 5}s`,
        price: (time - 5) * 200,
      });
    if (chars > 1)
      details.push({
        name: `${_d2.field_chars || "人物數量"} x${chars}`,
        price: (chars - 1) * 1000,
      });
    if (document.getElementById("processPerformance").checked)
      details.push({ name: _d2.field_performance || "表演設計", price: 1000 });
    if (document.getElementById("processBg").checked)
      details.push({ name: _d2.field_bg || "背景動畫", price: 500 });
    let cost =
      baseCost +
      physics +
      (time > 5 ? (time - 5) * 200 : 0) +
      (chars > 1 ? (chars - 1) * 1000 : 0);
  ...
  if (
    document.getElementById("animRush") &&
    document.getElementById("animRush").checked
  ) {
    let rPrice =
      parseInt(document.getElementById("animRushPrice").value) || 1000;
    extra += rPrice;
    details.push({ name: _d2.opt_rush || "加急趕工", price: rPrice });
  }

  let subtotal = extra;
  let fee = 0;
  let paymentMethod = _d2.pay_bank || "銀行匯款";
  if (document.getElementById("animPaypal").checked) {
    let r = parseFloat(document.getElementById("animPaypalRate").value) || 0;
    fee += Math.round(subtotal * (r / 100));
    paymentMethod = "PayPal";
    if (fee > 0)
      details.push({
        name: `PayPal ${_d2.pay_fee || "手續費"} (${r}%)`,
        price: fee,
      });
  } else if (document.getElementById("animEcpay").checked) {
    let r = parseFloat(document.getElementById("animEcpayRate").value) || 0;
    fee += Math.round(subtotal * (r / 100));
    paymentMethod = _d2.pay_ecpay || "綠界/超商";
    if (fee > 0)
      details.push({
        name: `${_d2.pay_ecpay || "綠界"} ${_d2.pay_fee || "手續費"} (${r}%)`,
        price: fee,
      });
  }
  ...
  const plan = document.querySelector(
    'input[name="animPaymentPlan"]:checked',
  ).value;
  if (plan === "two") {
    let f = Math.round(finalTotal * 0.03);
    finalTotal += f;
    details.push({
      name: `${_d2.pay_two || "兩期"} ${_d2.pay_fee || "手續費"} (3%)`,
      price: f,
    });
  } else if (plan === "three") {
    let f = Math.round(finalTotal * 0.05);
    finalTotal += f;
    details.push({
      name: `${_d2.pay_three || "三期"} ${_d2.pay_fee || "手續費"} (5%)`,
      price: f,
    });
  }
  ...
  let animPayHtml = "";
  if (animPlan === "one") {
    animPayHtml = `${_d2.pay_one || "一次付清"}：<strong class="text-emerald-400">${getCurrencyPrefix()}${formatMoney(finalTotal)}</strong>`;
  } else {
    animPayHtml = `${_d2.pay_installment || "分期付款"}：<strong class="text-blue-400">${_d2.deposit_label || "頭款"} ${getCurrencyPrefix()}${formatMoney(Math.ceil(finalTotal / (animPlan === "two" ? 2 : 3)))}</strong>`;
  }

  document.getElementById("animDepositInfo").innerHTML = animPayHtml;
  document.getElementById("animTotalPrice").textContent =
    `${getCurrencyPrefix()}${formatMoney(finalTotal)}`;
  document.getElementById("animDetailList").innerHTML = details
    .map(
      (i) =>
  ...
  window.currentAnimDetails = details;
  window.currentAnimTotal = finalTotal;
  window.currentAnimPayment = paymentMethod;
  window.currentAnimPlan = plan;
  ...
  if (animRushBlock) {
    animRushBlock.classList.toggle("hidden", !animRushText.trim());
    document.getElementById("animQuoteRushSummaryText").textContent =
      animRushText;
  }
  if (animSuppBlock) {
    animSuppBlock.classList.toggle("hidden", !animSuppText.trim());
    document.getElementById("animQuoteSuppSummaryText").textContent =
      animSuppText;
  }
  syncCheckboxVisuals();
...
function resetAnimForm() {
  document
    .querySelectorAll('#page-anim input[type="checkbox"]')
    .forEach((i) => (i.checked = false));

  const defaultType = document.querySelector(
    '#page-anim input[name="animBaseType"][value="sticker"]',
  );
  if (defaultType) defaultType.checked = true;
  const defaultPlan = document.querySelector(
    '#page-anim input[name="animPaymentPlan"][value="one"]',
  );
...
const calculateAnimDebounced = debounce(function () {
  if (typeof calculateAnim === "function") calculateAnim();
}, 150);

function selectPaymentAnim(s) {
  ["bank", "paypal", "ecpay"].forEach(
    (p) =>
      (document.getElementById(
        "anim" + p.charAt(0).toUpperCase() + p.slice(1),
      ).checked = p === s),
  );
  calculateAnim();
  syncCheckboxVisuals();
...
function toggleAnimRushField() {
  document
    .getElementById("animRushContainer")
    .classList.toggle("hidden", !document.getElementById("animRush").checked);
  calculateAnim();
}
