(function () {
  ...
  function renderGifGrid() {
    ...
    grid.innerHTML = slice
      .map(function (o, i) {
        ...
        return (
          '<div class="carousel-img-wrap relative cursor-pointer rounded-2xl overflow-hidden border ' +
          borderCls +
          ' bg-black/40 group tmpl-card-hover" style="aspect-ratio:1;' +
          hoverStyle +
          '" onclick="tmplToggleOutfitFromGif(' +
          realIdx +
          ')">' +
          '<img src="' +
          o.src +
          '" alt="' +
          o.name +
          '" class="carousel-img w-full h-full object-cover" style="transform:translate3d(0,0,0);will-change:transform;transition:transform 0.3s cubic-bezier(0.34,1.56,0.64,1);" loading="lazy">' +
          ...
          '<span class="text-[0.65rem] font-bold text-white leading-tight block">' +
          o.name +
          "</span>" +
          "</div>" +
          (isChecked
            ? '<div class="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-fuchsia-500 flex items-center justify-center shadow-lg shadow-fuchsia-500/50 check-badge-pop"><i class="fa-solid fa-check text-white text-[0.55rem]"></i></div>'
            : "") +
          "</div>"
        );
      })
  ...
  window.tmplGifGoPage = function (p) {
    gifPage = p;
    renderGifGrid();
  };
  ...
  function renderCheckGrid() {
    ...
    grid.innerHTML = window.TMPL_OUTFITS.map(function (o, idx) {
      ...
      return (
        ...
        '<div class="check-box">' +
        (checked
          ? '<i class="fa-solid fa-check text-white text-[0.6rem] check-badge-pop"></i>'
          : "") +
        "</div>" +
        "<span>" +
        o.name +
  ...
  window.tmplToggleOutfit = function (idx) {
    if (selectedOutfits.has(idx)) {
      selectedOutfits.delete(idx);
    } else {
      selectedOutfits.add(idx);
    }
    renderCheckGrid();
    renderGifGrid();
    tmplCalculate();
  ...
  window.tmplSwitchModel = function (model) {
    ...
    tmplQty.acc = 0;
    tmplQty.expr = 0;
    ["tmplHairBang", "tmplHairFull", "tmplAcc", "tmplExpr"].forEach(
      function (id) {
        var el = document.getElementById(id);
        if (el) el.checked = false;
      },
    );
    ["tmplHairBangQty", "tmplHairFullQty", "tmplAccQty", "tmplExprQty"].forEach(
      function (id) {
        var el = document.getElementById(id);
  ...
  var CHANYE_GIFS = [
    {
      src: "https://pbs.twimg.com/profile_images/2002045915809656832/kDFrBERD_400x400.png",
      label: "長夜月搖 展示圖 1",
    },
    {
      src: "https://pbs.twimg.com/profile_images/2002045915809656832/kDFrBERD_400x400.png",
  ...
  function renderChanyeGifGrid() {
    ...
    grid.innerHTML = slice
      .map(function (o) {
        return (
          '<div class="relative rounded-2xl overflow-hidden border border-white/10 bg-black/40 tmpl-card-hover" style="aspect-ratio:3/4;cursor:default;">' +
          '<img src="' +
          o.src +
          '" alt="' +
          o.label +
          '" class="w-full h-full object-cover" style="transform:translate3d(0,0,0);will-change:transform;" loading="lazy">' +
  ...
  window.tmplChanyeGifGoPage = function (p) {
    chanyeGifPage = p;
    renderChanyeGifGrid();
  };
  ...
  window.tmplToggleQty = function (id) {
    var key = id
      .replace("tmpl", "")
      .replace("HairBang", "hairBang")
      .replace("HairFull", "hairFull")
      .replace("Acc", "acc")
      .replace("Expr", "expr");
    ...
    if (el && el.checked) {
      if (tmplQty[key] === 0) {
        tmplQty[key] = 1;
        var qEl = document.getElementById(id + "Qty");
        if (qEl) qEl.textContent = 1;
      }
    } else {
      tmplQty[key] = 0;
      var qEl2 = document.getElementById(id + "Qty");
      if (qEl2) qEl2.textContent = 0;
    }
  ...
  window.tmplChangeQty = function (key, delta) {
    var idMap = {
      hairBang: "tmplHairBang",
      hairFull: "tmplHairFull",
      acc: "tmplAcc",
      expr: "tmplExpr",
    };
    ...
    var qtyIdMap = {
      hairBang: "tmplHairBangQty",
      hairFull: "tmplHairFullQty",
      acc: "tmplAccQty",
      expr: "tmplExprQty",
      chanyeQty: "tmplChanyeQtyDisplay",
    };
    var qEl = document.getElementById(
      qtyIdMap[key] ||
        "tmpl" + key.charAt(0).toUpperCase() + key.slice(1) + "Qty",
    );
  ...
  window.tmplCalculate = function () {
    ...
    if (count >= 1) {
      var selectedNames = Array.from(selectedOutfits)
        .sort(function (a, b) {
          return a - b;
        })
        .map(function (i) {
          return window.TMPL_OUTFITS[i].name;
        });
      var _dCalc =
        ...
        I18N[currentLang]
          ? I18N[currentLang]
          : {};
      var outfitDetailName =
        (_dCalc.tmpl_detail_outfit || "模板衣裝") +
        " × " +
        count +
        " " +
        (_dCalc.tmpl_detail_suit || "套");
      details.push({
        name: outfitDetailName,
        price: outfitCost,
        subItems: selectedNames,
      });
    }
    ...
    if (tmplCurrentModel === "chanye" && tmplQty.chanyeQty >= 1) {
      ...
      details.push({
        name:
          (_dCh.tmpl_chanye_qty_detail || "長夜月搖擺") +
          " × " +
          tmplQty.chanyeQty +
          " " +
          (_dCh.tmpl_chanye_qty_unit || "隻"),
        price: chanyeCost,
      });
    }
    var _d =
      ...
      if (details.length === 0) {
        var _dEm =
          ...
          I18N[currentLang]
            ...
            : {};
        dl.innerHTML =
          '<div class="text-xs text-purple-400/50 text-center py-2">' +
          (_dEm.tmpl_detail_empty || "尚未選擇任何項目") +
          "</div>";
      } else {
        dl.innerHTML = details
          .map(function (i) {
            ...
            if (i.subItems && i.subItems.length > 0) {
              sub =
                ...
                i.subItems
                  .map(function (n) {
                    return (
                      '<li style="color:rgba(196,181,253,0.6);font-size:0.68rem;display:flex;align-items:center;gap:4px;"><span style="opacity:0.5">—</span>' +
                      n +
                      "</li>"
                    );
                  })
            ...
            return (
              ...
              '<span class="text-purple-200/80 text-xs">' +
              i.name +
              "</span>" +
              '<span class="text-white font-bold text-xs ml-2 shrink-0">NT$ ' +
              fmt(i.price) +
              "</span>" +
              "</div>" +
              sub +
    ...
    if (tp) {
      ...
      if (_tpFrom !== total) {
        if (
          _tmplFirstAmountRender &&
          typeof window.whenPanelVisible === "function"
        ) {
          _tmplFirstAmountRender = false;
          window.whenPanelVisible(tp, function () {
    ...
    if (dep && total > 0)
      dep.innerHTML =
        '<span class="text-purple-300/70">' +
        (_dDep.tmpl_deposit_label || "訂金（50%）") +
        '：</span><strong class="text-white">NT$ ' +
        fmt(Math.ceil(total * 0.5)) +
        "</strong>";
    else if (dep) dep.innerHTML = "";

    var accDesc = (document.getElementById("tmplAccDesc") || {}).value || "";
    var _suppEl =
      tmplCurrentModel === "chanye"
        ? document.getElementById("tmplChanyeSuppInfo")
        : document.getElementById("tmplSuppInfo");
  ...
  window.tmplReset = function () {
    ...
    tmplQty.chanyeQty = 1;
    var chQtyEl = document.getElementById("tmplChanyeQtyDisplay");
    if (chQtyEl) chQtyEl.textContent = "1";
    ["tmplHairBang", "tmplHairFull", "tmplAcc", "tmplExpr"].forEach(
      function (id) {
        var el = document.getElementById(id);
        if (el) el.checked = false;
    ...
    var w = document.getElementById("tmplAccDescWrap");
    if (w) w.classList.add("hidden");
    var ad = document.getElementById("tmplAccDesc");
    if (ad) ad.value = "";
    var si = document.getElementById("tmplSuppInfo");
    if (si) si.value = "";
    var si2 = document.getElementById("tmplChanyeSuppInfo");
    if (si2) si2.value = "";
    var at = document.getElementById("tmplAgreeTerms");
  ...
  window.tmplCopySummary = function () {
    ...
    var lines = [
      "💜 " +
        (_dCopy.nav_template || "聯名模板") +
        " — " +
        (_dCopy.tmpl_copy_header || "委託需求清單") +
        " 💜",
      "================================",
      "【" +
        (_dCopy.tmpl_copy_type || "模板類型") +
        "】" +
        (tmplCurrentModel === "chanye"
          ? _dCopy.tmpl_card_chanye_name || "長夜月搖（紅妻）"
          : _dCopy.tmpl_card_kanso_name || "換裝娃娃（殘光）"),
      "【" +
        (_dCopy.tmpl_copy_outfits || "選擇衣裝") +
        "】" +
        (selectedNames.length
          ? selectedNames.join("、")
          : _dCopy.tmpl_copy_none || "（未選擇）"),
      "--------------------------------",
    ];
    ...
    if (window.tmplCurrentAccDesc && window.tmplCurrentAccDesc.trim()) {
      ...
      lines.push(
        "【" +
          (_dCopy.tmpl_copy_acc || "配件需求") +
          "】" +
          window.tmplCurrentAccDesc.trim(),
      );
    }
    if (window.tmplCurrentSupp && window.tmplCurrentSupp.trim()) {
      ...
      lines.push(
        "【" +
          (_dCopy.tmpl_copy_supp || "補充說明") +
          "】" +
          window.tmplCurrentSupp.trim(),
      );
    }
    lines.push("");
    lines.push(
      "※ " + (_dCopy.tmpl_copy_note || "以上為初步估算，實際報價以確認後為準"),
    );
    var text = lines.join("\r\n");
