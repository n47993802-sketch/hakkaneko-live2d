   ...
   修掉舊版每個頁面都寫死 tab-intro 為 active 的問題。 */
    (function() {
        function currentPageId() {
            // 根目錄頁面（index.html）本身 SITE_BASE 一定是空字串，
            // 用這個最直接分辨「現在是不是首頁」，不用去猜路徑深度。
            if (!window.SITE_BASE) return 'intro';
            var segments = location.pathname.split('/').filter(Boolean);
            if (segments.length && /\.html$/i.test(segments[segments.length - 1])) {
                var last = segments.pop();
                // 正常情況（乾淨網址，資料夾底下是 index.html）：
                // 拿掉 index.html 後，剩下最後一段資料夾名稱就是 pageId。
                // 相容情況（本機測試時直接打開 xxx.html 檔名）：
                // 沒有 index.html 可拿掉，直接用檔名（去掉副檔名）當 pageId。
...
/* ── 核心邏輯：分頁切換、報價計算機、主題/語言切換等 ── */
        ...
        // 統一燈箱系統 (支援左右切換 + 圓點導航)
        // ══════════════════════════════════════════════════
        // stickers／logos 這兩組不再寫死於此，改由 portfolio-render.js
        // 於載入時直接指向 window.PORTFOLIO_MEDIA（見該檔案），避免跟
        // portfolio-config.js 維護兩份重複、容易對不起來的資料。
        // fanart 一樣由 loadFanart() 動態填入。
        const ulbGroups = {
            stickers: [],
            logos:    [],
            fanart:   []
        };
        // stickers／logos 裡的項目是 {src, poster, ...} 物件（影片），
        // fanart 裡的項目是純字串網址（圖片）——ulbRender 依此分流。
        ...
        // ==================== 動態貼圖 / Logo 展示：資料 ====================
        // 已拆分至 portfolio-config.js（window.PORTFOLIO_MEDIA /
        // window.PORTFOLIO_PER_PAGE），只有 portfolio.html 會載入這個檔案；
        // gifPage（目前分頁狀態）搬到 portfolio-render.js 裡。
