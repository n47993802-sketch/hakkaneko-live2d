...
| `images/rules/` | rules.html（流程與規範） | （目前多為文字，無大量圖片） | 流程說明用的示意圖（如有） |
| `images/core/` | core.html（V皮設計） | （目前多為文字/表格） | V皮設計方案示意圖（如有） |
| `images/anim/` | anim.html（動畫設計） | （目前多為文字/表格） | 動畫設計方案示意圖（如有） |
| `images/template/` | template.html（聯名模板） | `HakkaNeko/CollabTemplateA/01.png` ~ `30.png` | 30 款衣裝縮圖，`common.js` 內 `TMPL_OUTFITS` 陣列 |
| `images/portfolio/cover/` | portfolio.html（作品展示・V皮/動畫作品） | `HakkaNeko/cover%20image/*.png、*.webp` | 作品展示的封面照片 |
| `images/portfolio/stickers/` | portfolio.html（作品展示・動態貼圖） | `HakkaNeko/DynamicStickers/*.gif` | `common.js` 內 `GIF_DATA.stickers` |
| `images/portfolio/logos/` | portfolio.html（作品展示・動態Logo） | `HakkaNeko/DynamicLogo/*.gif` | `common.js` 內 `GIF_DATA.logos` |
| `images/channels/` | channels.html（推薦頻道） | Twitch/YouTube 頻道圖示（多為外部 CDN，通常不需要搬） | 頻道相關圖片（如有） |
| `images/fanart/` | fanart.html（二創展示） | `HakkaNeko/Second%20creation%20drawing/*`（目前用 GitHub API **動態抓取**，不是寫死清單） | 二創圖片 |
| `images/live2d-demo/` | live2d-demo.html（角色互動，目前隱藏中） | 尚未實作 | 預留 |
...
> `fanart.html` 比較特別：它不是寫死圖片清單，而是用 GitHub API
...
2. 打開 `common.js`，找到 `TMPL_OUTFITS`、`GIF_DATA` 這兩個陣列，
   或是 `portfolio.html`、`fanart.html` 裡的 `<img src="...">`，
   把網址從
