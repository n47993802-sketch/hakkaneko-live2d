/* ============================================================
   ...
   關於 window.SITE_BASE（v56 乾淨網址調整後更新）：
   - nav-config.js 裡的 href 都是「從網站根目錄算起」、結尾帶斜線、
     不含檔名的資料夾路徑，例如 'commission/rules/'（首頁例外，
     href 是空字串 ''）。頁面實際上有三種深度：
     index.html 在根目錄（深度0）；artists/index.html 在一層資料夾
     下（深度1）；其餘 8 個頁面都在 commission/xxx/ 或 creative/xxx/
     底下（深度2）。
   - 每個頁面的 <body> 一開頭都寫了 window.SITE_BASE：index.html 是
     ''，artists/index.html 是 '../'，其餘深度2的頁面是 '../../'。
     這裡產生連結時，一律讀取這個值
